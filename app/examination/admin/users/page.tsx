...
const handleCopy = (rollNo: string, password?: string, designation?: string) => {
...
const Avatar = ({ designation, rollNo }: { designation?: string; rollNo?: string }) => {
  const safe = rollNo || "?";
  const initials = designation
    ? designation.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2)
    : safe[0].toUpperCase();
  const colors = [
...
export default function AdminUserManager() {
  ...
  const handleCreateUser = async () => {
    ...
    try {
      const res = await fetch("/examination/api/admin/users", {
        ...
        body: JSON.stringify({ rollNo, designation, password }),
      });
      const data = await res.json();
      if (res.ok) {
        setRollNo("");
  ...
  const handleCopyWithFeedback = (
    ...
    handleCopy(rollNo, password, designation);
    setCopied(userId);
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-50 flex items-center gap-2.5">
              ...
              <h2 className="text-sm font-semibold text-gray-800">Create New User</h2>
            </div>
          ...
          {editingUser && (
            <div className="bg-white rounded-2xl border border-amber-100 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-amber-50 flex items-center gap-2.5 bg-amber-50">
                ...
                <h2 className="text-sm font-semibold text-amber-800">Edit User</h2>
              </div>
            ...
            <div className="overflow-y-auto flex-1 divide-y divide-gray-50">
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((user) => (
                  <div key={user._id} className="px-5 py-4 hover:bg-gray-50/70 transition-colors group">
                    <div className="flex items-start gap-3">
                      <Avatar designation={user.designation} rollNo={user.rollNo} />
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-2.5">
                          <button
                            onClick={() => handleCopyWithFeedback(user._id, user.rollNo || "", user.password, user.designation)}
