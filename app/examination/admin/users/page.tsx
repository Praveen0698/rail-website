...
export default function AdminUserManager() {
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await fetch("/examination/api/admin/send-credentials", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            email: data.email,
  ...
  const handleCopyWithFeedback = (
    ...
    handleCopy(email, password, name);
    setCopied(userId);
  ...
  const InputField = ({
    ...
    <input
      ...
      value={value}
      onChange={onChange}
      className={`w-full border border-gray-200 bg-gray-50 px-3.5 py-2.5 rounded-lg text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-violet-400 focus:border-transparent focus:bg-white transition-all ${className}`}
    />
  );
