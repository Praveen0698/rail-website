...
// ─── Generate a unique 10–12 digit roll number on the frontend ───
...
const RollNumberModal = ({
  ...
  <div className="bg-white w-full max-w-md border border-black shadow-lg p-6">
    
    {/* Header */}
    <h2 className="text-center font-bold text-[15px] uppercase tracking-wide border-b pb-3 mb-4">
    ...
    <div className="text-center mb-5">
      <p className="text-sm font-semibold mb-2">
        Your Application Roll Number
      </p>

      <div className="flex items-center justify-center gap-2">
        <span className="text-2xl font-bold tracking-widest border rounded-md border-black px-4 py-2 bg-gray-50 select-all">
          {rollNumber}
        </span>
        ...
        <button
          onClick={handleCopy}
          className="flex items-center justify-center w-9 h-9 border border-black rounded-md 
    ...
    {/* Instructions */}
    <div className="border border-black p-3 mb-5 text-[12px] leading-relaxed bg-yellow-50">
      <p className="font-bold mb-1 text-[13px]">
        ⚠ Important Instructions:
      </p>
      <ul className="list-disc pl-4 space-y-1">
        <li>
          This Roll Number is generated <strong>only once</strong>. Please
          note it down carefully.
        </li>
        <li>
          You will need this Roll Number for all future correspondence
          regarding your application.
        </li>
        <li>
          Click <strong>{`${"Save & Submit"}`}</strong> to confirm
          and save your application. This action cannot be undone.
        </li>
        <li>
          If you click <strong>{`${"Cancel"}`}</strong>, your application
          will <strong>not</strong> be saved and a new Roll Number will be
          generated on your next attempt.
        </li>
      </ul>
    </div>
    ...
    <div className="flex gap-3 justify-center">
      <button
        onClick={onCancel}
        disabled={saving}
        className="px-6 py-2 border border-black rounded-md text-sm hover:bg-gray-100 transition-colors disabled:opacity-50"
      >
        Cancel
      </button>
      ...
      <button
        onClick={onSave}
        disabled={saving}
        className="px-6 py-2 bg-blue-600 text-white text-sm rounded-md 
...
const PageTwo = () => {
  ...
  const [pendingRollNumber, setPendingRollNumber] = useState<string>("");

  const buildFormData = (rollNumber: string) => ({
    name: nameGrid
      .map((r) => r.join(""))
      .join("")
      .trim(),
    ...
    rollNumber, // ✅ passed from frontend
  });

  // Step 1: validate → generate roll number → show modal
  const handleSubmit = () => {
    const name = nameGrid
      .map((r) => r.join(""))
      .join("")
      .trim();
    const fatherName = fatherNameGrid
      .map((r) => r.join(""))
      .join("")
      .trim();
    ...
    setPendingRollNumber(roll);
    setShowModal(true);
  };

  // Step 2: user clicks "Save & Submit" in modal → actual API call
  const handleConfirmSave = async () => {
    setSubmitting(true);
    try {
      const formData = buildFormData(pendingRollNumber);
      ...
      setShowModal(false);
      alert(
        `Application submitted successfully ✅\nYour Roll Number: ${pendingRollNumber}`,
      );
  ...
  // Step 2 (cancel): close modal, discard roll number
  ...
  return (
    <>
      {/* ── Roll Number Confirmation Modal ── */}
      {showModal && (
        <RollNumberModal
      ...
      <div className="bg-white w-full max-w-4xl mx-auto p-4 sm:p-8 md:p-12 lg:p-20 text-[14px] leading-relaxed">
            ...
            <label className="relative w-[4cm] h-[5cm] border p-2.5 flex items-center justify-center cursor-pointer overflow-hidden shrink-0">
              ...
              {photo ? (
                ...
                <span className="text-[10px] text-center text-gray-400">
                  Upload your recent passport size black & white photo, not
                  older than one month as on date of application. Photo to be
                  sharp & clear, with light background, suitable for scanning
                  and printing. Photo to be in 4cm x 5cm size. Photo to be
                  pasted firmly
                </span>
              )}
            </label>
        ...
        <div className="mb-4">
          <p className="font-semibold mb-2 w-full">
            ...
            <span className="font-normal">
              ...
              the Application Form and Personal Data Sheet are true, complete
              and correct to the best of my knowledge and belief. In the event
              of any discrepancy in the particulars or any statement being found
              false at any stage, my candidature / service would be cancelled /
              terminated without any notice.&quot; (Above declaration is to be
              written below in the applicant&apos;s own running script in
              English):{" "}
            </span>
          </p>
        ...
        <div className="mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <div className="flex flex-row gap-1 items-end mb-1">
              ...
              <input
                type="text"
                className="w-1/2 p-1.5 outline-none border-b"
              />
            </div>
            <div className="flex flex-row gap-1 items-end mb-1">
        ...
        <div className="mt-6 flex justify-center">
          <button
            ...
            disabled={submitting}
            className={`px-10 py-2.5 border border-black font-semibold tracking-wide cursor-pointer
